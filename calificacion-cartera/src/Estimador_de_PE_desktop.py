...
class PEApp:
    def __init__(self, root):
        ...
        lbl_title = ttk.Label(main_frame, text="Estimador de Pérdida Esperada (PE)", font=("Helvetica", 16, "bold"))
        lbl_title.pack(pady=(0, 10))
        
        lbl_desc = ttk.Label(main_frame, text="Procesamiento de archivos de cartera y aplicación de modelos ML.", wraplength=600)
        lbl_desc.pack(pady=(0, 20))

        # Selector Carpeta Entrada
        frm_input = ttk.LabelFrame(main_frame, text="Carpeta de Archivos (Entrada)", padding="10")
        ...
        btn_input = ttk.Button(frm_input, text="📂 Buscar", command=lambda: self.seleccionar_carpeta(self.carpeta_base))
        btn_input.pack(side=tk.RIGHT)

        # Selector Carpeta Salida
        frm_output = ttk.LabelFrame(main_frame, text="Carpeta Salida (Resultados)", padding="10")
        ...
        btn_output = ttk.Button(frm_output, text="📂 Buscar", command=lambda: self.seleccionar_carpeta(self.carpeta_salida))
        btn_output.pack(side=tk.RIGHT)

        # Botón Ejecutar
        self.btn_run = ttk.Button(main_frame, text="▶ Ejecutar Estimación", command=self.iniciar_proceso)
        ...
        self.txt_logs = scrolledtext.ScrolledText(main_frame, height=12, state='disabled', font=("Consolas", 9))
    ...
    def procesar_datos(self, carpeta_base, carpeta_salida):
        try:
            ...
            self.log(f"Archivos encontrados: {len(archivos_credito)} créditos, Asociados, Aportes ({'Sí' if archivo_aportes else 'No'}), Captaciones ({'Sí' if archivo_captaciones else 'No'}).")
            ...
            for i, archivo in enumerate(archivos_credito):
                try:
                    ...
                    if missing_cols:
                        self.log(f"Archivo {archivo} incompleto. Faltan: {missing_cols}", "warning")
                        continue
            ...
            meses_ordenados = sorted(dataframes.keys(), key=lambda x: mes_ano_to_date(x, self.log))
            ...
            df_mora = df_mora.rename(columns=lambda x: x + suffix if x in ['Morosidad', 'CodigoContable', 'NIT', 'SALPRES', 'VEA', 'clasegarantia'] else x)
            df_mora = df_mora.rename(columns={'clasegarantia': f'clasegarantia{suffix}'}) # Asegurar renombrado si no cae en lambda
            ...
            df_mora = dataframes[meses_ordenados[0]][['NIT', 'Morosidad', 'CodigoContable', 'SALPRES', 'VEA', 'clasegarantia']].rename(columns=cols_rename)


            for i, mes in enumerate(meses_ordenados[1:]):
                df_temp = dataframes[mes][['NIT', 'Morosidad', 'CodigoContable', 'SALPRES', 'VEA', 'clasegarantia']].rename(columns={
            ...
            try:
                ultimo_dia_mes = mes_ano_to_date(last_month, self.log) + pd.offsets.MonthEnd(0)
            except:
            ...
            if archivo_captaciones:
                ...
                df_captaciones = df_captaciones[df_captaciones['es CDAT']==1][['NIT', 'COOCDAT']]
