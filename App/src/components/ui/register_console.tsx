...
export default function RegisterConsole({ logs, onClear }: RegisterProps) {
  ...
  return (
    <section className="card log-card">
      ...
      <div
        className="log-card-console"
        role="log"
        aria-live="polite"
        aria-label="Registro de ejecución"
      >
        {logs.length === 0 ? (
          <p className="log-card-empty">
            Sin registros. Ejecute la estimación para ver el progreso.
          </p>
        ) : (
          logs.map((entry, i) => (
            <div key={i} className="log-entry">
              ...
              <span
                ...
                style={{ color: LOG_COLORS[entry.level] ?? "#9ca3af" }}
              >
                {entry.level}:
              </span>{" "}
